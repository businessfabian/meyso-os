...
- [ ] 👤 Dave | Demo-Kennzeichen: die Auslegung von "ausser an sich selbst" bestaetigen oder korrigieren. Umgesetzt ist: die Adresse des Kunden ist die Firmenadresse oder die eines aktiven Admin-Zugangs, geprueft wird die heutige Adresse (anSichSelbst in lib/kunde-zaehlt.ts). Folge aus dem Review: wird die E-Mail eines Kunden auf eine eigene geaendert, sind auch aeltere Rechnungen freigegeben, setzen darf das Kennzeichen trotzdem nur inhaber
