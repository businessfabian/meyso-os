...
- [ ] 👤 Dave | PR 73 (Dialoge) entscheiden: Merge, keine Migration. Dazu offen: ein Verhaltenstest der Huelle in der Suite (zwei Dialoge, Escape, Fokus) braucht jsdom und Testing Library, also eine neue Abhaengigkeit. Bis dahin prueft scripts/dialog-bilder.mjs --probe das Verhalten im echten Browser
- [ ] 🤖 Auftraege-Runde: vier Zustaende und pausiert als Kennzeichen, Menue der Zustaende mit Wirkung, Wartungsvertrag-Dialog auf V2, Waechterpruefung 22, Auftragsliste ohne Umbruch (22.09.2026, PR 74, gebaut, Migration und Merge nach Daves Wort) Zustaende in_arbeit, abgenommen, live, abgeschlossen, Reihenfolge nach AGB § 5, live nur nach der Abnahme, abgeschlossen nur ohne offene Rechnung. Der Wartungsvertrag entsteht als Entwurf mit Typ aus der Vormerkung, Betrag je Intervall, Frist 30 Tage zum Monatsende, Minuten je Typ und AGB-Fassung, Festschreiben ist der zweite Klick und macht ihn aktiv. Pruefung 22 meldet einen Entwurf mehr als 7 Tage nach der Abnahme
...
- [ ] 👤 Dave | Migration 20260922_auftraege_zustaende.sql im SQL-Editor einspielen, nach dem Merge von PR 73 und vor dem Merge von PR 74, danach die Katalog-Nachher-Liste aus dem Kopf der Datei. Neue Spalte auftraege.pausiert, zwei Bedingungen ersetzt, heute_dashboard neu mit live bei der Schlussrechnung. Bestand: Webseite (planung) wird in_arbeit, AN-2026-003 bleibt abgenommen
- [ ] 👤 Dave | PR 74 (Auftraege-Runde) entscheiden, Merge erst nach der Migration. Zu wissen: Festschreiben macht jetzt einen Entwurf aus einem Auftrag aktiv, jeden anderen Entwurf nicht (bisher galt ausnahmslos: festgeschrieben heisst nicht geschlossen). Jaehrlich abgerechnet laeuft der Vertrag zwoelf Monate mit Frist zum Laufzeitende (AGB § 14 Abs. 2), monatlich 30 Tage zum Monatsende
- [ ] 🤖 Wartungsvertraege aus Finanzen (Neuer Vertrag) und aus "Neuer Kunde" bekommen 0 statt 30 Aenderungsminuten, weil die Routen minuten_monat nicht setzen und die Spalte auf 0 faellt. Aufgefallen in der Auftraege-Runde, dort nur fuer den Weg aus dem Auftrag behoben. Bestehende Vertraege lesend pruefen, Korrektur als Migration fuer Dave
...
- [ ] 🤖 Fristen auf HEUTE: dringende Fristen aus Dokumenten und Lieferantenvertraegen erscheinen fuer keine Rolle als Punkt, weil company_documents und supplier_contracts in PUNKT_QUELLEN (lib/heute-sicht.ts) fehlen und der Rollenfilter sie deshalb fuer jede Rolle wegnimmt. Die kommenden Fristen stehen weiter unter den Terminen. Aufgefallen am 21.09.2026 in Tempo 4, dort bewusst nicht geaendert. Zu klaeren, ob Absicht
