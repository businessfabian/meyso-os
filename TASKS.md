...
- [ ] 👤 Dave | PR 84 (Kosten nach Abfluss) entscheiden. Dabei: Kosten bisher und damit das Soll der Steuerruecklage rechnen mit dem Gebuchten, eine Ausgabe ohne Zahlungsdatum zaehlt erst mit dem Abfluss
- [ ] 🤖 Nachlese PR 84, ohne Termin: expenses erlaubt vertrag_id ohne periode. Eine solche Buchung markiert ihre Periode nicht als gebucht, Ausgabenlauf und Prognose zaehlten die Periode noch einmal. Heute gibt es keine solche Zeile (alle 16 Buchungen mit Periode, gelesen am 25.09.2026). Vorschlag: Migration CHECK (vertrag_id IS NULL OR periode IS NOT NULL) auf expenses, nach Daves Wort
